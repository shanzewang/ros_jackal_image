...
def initialize_policy(config, env, init_buffer=True, device=None):
    ...
    if device is None:
        ...
        print(f"found device: {[device for device in devices]}")
...
def train(env, policy, replay_buffer, config):
    ...
    n_iter = 0
    n_ep = 0
    epinfo_buf = collections.deque(maxlen=10)   # from 300 to 10 
    ...
    while n_steps < training_args["max_step"]:
        ...
        if n_iter % training_config["log_intervals"] == 0:
            ...
            policy.save(save_path, str(save_step)+"last_policy")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_path', dest='config_path', default="/home/eias/sac_final/jackal_ws/src/ros_jackal/configs/e2e_default_SAC.yaml")
